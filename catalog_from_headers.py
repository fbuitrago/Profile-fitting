...
single_or_double = 2  #single (1) or double (2) component fit
number_ini_conds = 2
path_for_galaxy_images = "../../"
#use only if different_fields == False
pix_scale = 0.339                          #in case different_fields = False
stars = np.array(["star319149"],dtype=str) #in case different_fields = False
...
for ii in range(len(galaxies)):
    ...
    for jj in range(len(stars)):
        for cont_ini_cond in range(number_ini_conds):
            for kk in range(len(masks)):
                #for cont in range(len(ini_conds)):
                filename = "./"+galaxies[ii]+"/"+galaxies[ii]+"_"+stars[jj]+"_"+masks[kk]+"_"+str(cont_ini_cond)+".fits"
                print(filename)
...
for gal_dict in gal_list:
    ...
    if single_or_double == 1:
        is_good_analysis = mag_flag_special_array[::2] + re_flag_special_array[::2] + nn_flag_special_array[::2] + ar_flag_special_array[::2] + pa_flag_special_array[::2]
    else:
        is_good_analysis = mag_flag_special1_array[::2] + re_flag_special1_array[::2] + nn_flag_special1_array[::2] + ar_flag_special1_array[::2] + pa_flag_special1_array[::2] + \
                           mag_flag_special2_array[::2] + re_flag_special2_array[::2] + nn_flag_special2_array[::2] + ar_flag_special2_array[::2] + pa_flag_special2_array[::2]
    ...
    vector_index    = np.append(vector_index   ,index)
    ...
    img = fits.open(path_for_galaxy_images + "galaxy_images/"+galaxy["id"]+".fits")
    hdr = img[0].header
    xc = hdr["NAXIS1"]/2.
    yc = hdr["NAXIS2"]/2.
    #-------
    if single_or_double == 1:
        ...
        dist_target1 = np.sqrt( (xc-float(galaxy["xc1"][index]))**2. + (yc-float(galaxy["yc1"][index]))**2. )
        dist_target2 = np.sqrt( (xc-float(galaxy["xc2"][index]))**2. + (yc-float(galaxy["yc2"][index]))**2. )
        if dist_target1 > dist_target2: #I take the maximum distance as the distance to the target
    ...
    vector_flag_obj_detected = np.append(vector_flag_obj_detected,flag_obj_detected) #flag_obj_detected is True when the distance to the target's centroid is less or equal 5 pixels, i.e., is telling whether the object was detected


    if (flag_obj_detected == True) and (np.isnan(dist_target) != True):
        if single_or_double == 1:
            ...
            vector_mag_exist = np.append(vector_mag_exist, galaxy["mag_flag_special"][index] )
            vector_re_exist  = np.append(vector_re_exist,  galaxy["re_flag_special"] [index] )
            vector_nn_exist  = np.append(vector_nn_exist,  galaxy["nn_flag_special"] [index] )
            vector_ar_exist  = np.append(vector_ar_exist,  galaxy["ar_flag_special"] [index] )
            vector_pa_exist  = np.append(vector_pa_exist,  galaxy["pa_flag_special"] [index] )
        else:
            ...
            vector_magerr1    = np.append(vector_magerr1,np.std(galaxy["mag1"]) )
            ...
            vector_re1_kpc    = np.append(vector_re1_kpc,     galaxy["re1"][index]*pix_scale*kpc_per_arcsec)
            vector_re2_kpc    = np.append(vector_re2_kpc,     galaxy["re2"][index]*pix_scale*kpc_per_arcsec)
            ...
            vector_mag1_exist = np.append(vector_mag1_exist, galaxy["mag_flag_special1"][index] )
            vector_re1_exist  = np.append(vector_re1_exist,  galaxy["re_flag_special1"] [index] )
            vector_nn1_exist  = np.append(vector_nn1_exist,  galaxy["nn_flag_special1"] [index] )
            vector_ar1_exist  = np.append(vector_ar1_exist,  galaxy["ar_flag_special1"] [index] )
            vector_pa1_exist  = np.append(vector_pa1_exist,  galaxy["pa_flag_special1"] [index] )
            vector_mag2_exist = np.append(vector_mag2_exist, galaxy["mag_flag_special2"][index] )
            vector_re2_exist  = np.append(vector_re2_exist,  galaxy["re_flag_special2"] [index] )
            vector_nn2_exist  = np.append(vector_nn2_exist,  galaxy["nn_flag_special2"] [index] )
            vector_ar2_exist  = np.append(vector_ar2_exist,  galaxy["ar_flag_special2"] [index] )
            vector_pa2_exist  = np.append(vector_pa2_exist,  galaxy["pa_flag_special2"] [index] )
    else:
        if single_or_double == 1:
            ...
            vector_mag_exist = np.append(vector_mag_exist, float('NaN') )
            vector_re_exist  = np.append(vector_re_exist,  float('NaN') )
            vector_nn_exist  = np.append(vector_nn_exist,  float('NaN') )
            vector_ar_exist  = np.append(vector_ar_exist,  float('NaN') )
            vector_pa_exist  = np.append(vector_pa_exist,  float('NaN') )
        else:
            ...
            vector_magerr1     = np.append(vector_magerr1,    float('NaN') )
            ...
            vector_mag1_exist  = np.append(vector_mag1_exist, float('NaN') )
            vector_re1_exist   = np.append(vector_re1_exist,  float('NaN') )
            vector_nn1_exist   = np.append(vector_nn1_exist,  float('NaN') )
            vector_ar1_exist   = np.append(vector_ar1_exist,  float('NaN') )
            vector_pa1_exist   = np.append(vector_pa1_exist,  float('NaN') )
            vector_mag2_exist  = np.append(vector_mag2_exist, float('NaN') )
            vector_re2_exist   = np.append(vector_re2_exist,  float('NaN') )
            vector_nn2_exist   = np.append(vector_nn2_exist,  float('NaN') )
            vector_ar2_exist   = np.append(vector_ar2_exist,  float('NaN') )
            vector_pa2_exist   = np.append(vector_pa2_exist,  float('NaN') )
