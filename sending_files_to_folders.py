...
no_cpus = 3
no_galaxies = len( os.listdir("./galaxy_images/") )

gal,ra,dec = np.loadtxt("massive_high_z_bin_aegis.cat", dtype = float, unpack = True)
ids = ids_as_str(gal)

no_gals_per_directory = no_galaxies/no_cpus
    ...
    for jj in range(start_value,final_value): #+1 because range upper limit otherwise is not inclusive (it does -1)
        if ii is not no_cpus:
            ...
            if jj is final_value-1: #for the last element
                data = Table([ids[start_value:final_value],ra[start_value:final_value],dec[start_value:final_value]], names=['gal', 'ra', 'dec'])
                ascii.write(data,cpu_cat, Writer = ascii.CommentedHeader)
